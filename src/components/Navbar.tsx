...
const Navbar = () => {
    ...
    return (
        <div className="text-white/70 pt-6">
            <div className="hidden md:flex items-center px-4 py-2 mx-auto max-w-[400px]">
                <ul className="flex flex-row p-4 space-x-8">
                    {navLinks.map((link, index) => (
                        <li key={index}>
                            <Link href={link.path}><p>{link.title}</p></Link>
                        </li>
                    ))}
                    <li >
                        <a href="#contact" className="group">
                            ...
                            <div className="relative">
                                ...
                                <div className="mt-1 absolute w-2/4 h-1 transition-all duration-300 ease-out bg-orange-600 rounded-full group-hover:w-full"></div>
                            </div>
                        </a>
                    </li>
                </ul>
            ...
            <div
                onClick={toggleNav} className="md:hidden text-white/70 absolute top-5 right-5 border rounded border-white/70 p-2 z-50">
